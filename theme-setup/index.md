...
Most of the variables found here are used in the .html files found in `_includes` if you need to add or remove anything. A good place to start would be to add the `title`, `description`, and `url` for your site. Links are absolute and prefixed with `{% include url %}`.
...
![Twitter Card summary large image screenshot]({% include url %}/images/twitter-card-summary-large-image.jpg)
...
Link blog like a champ by adding `link: http://url-you-want-linked` to a post's YAML front matter. Arrow glyph links to the post's permalink and the the `post-title` links to the source URL. Here's an [example of a link post]({% include url %}/sample-link-post/) if you need a visual.
...
This theme is free and open source software, distributed under the [MIT License]({% include url %}/LICENSE) version 2 or later. So feel free to to modify this theme to suit your needs.
